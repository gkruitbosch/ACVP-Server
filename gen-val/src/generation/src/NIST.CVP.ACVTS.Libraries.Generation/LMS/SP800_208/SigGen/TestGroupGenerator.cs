...
public class TestGroupGenerator : ITestGroupGeneratorAsync<Parameters, TestGroup, TestCase>
{
    ...
    public async Task<List<TestGroup>> BuildTestGroupsAsync(Parameters parameters)
    {
        ...
        if (parameters.SpecificCapabilities?.Length > 0)
        {
            ...
            {
                var aftTestGroup = new TestGroup { LmsMode = cap.LmsMode, LmOtsMode = cap.LmOtsMode, TestType = ALGORITHM_FUNCTIONAL_TEST, MessageLength = parameters.MessageLength};
        ...
        else
        {
            ...
            foreach (var item in mappedOutputLengths)
            {
                ...
                for (var i = 0; i < maxLen; i++)
                {
                    ...
                    var aftTestGroup = new TestGroup { LmsMode = lmsMode, LmOtsMode = lmOtsMode, TestType = ALGORITHM_FUNCTIONAL_TEST,  MessageLength = parameters.MessageLength };
