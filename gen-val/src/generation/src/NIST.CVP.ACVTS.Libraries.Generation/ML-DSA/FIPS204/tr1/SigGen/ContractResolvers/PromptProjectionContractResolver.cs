...
namespace NIST.CVP.ACVTS.Libraries.Generation.ML_DSA.FIPS204.tr1.SigGen.ContractResolvers;
...
public class PromptProjectionContractResolver : ProjectionContractResolverBase<TestGroup, TestCase>
{
    ...
    protected override Predicate<object> TestCaseSerialization(JsonProperty jsonProperty)
    {
        ...
        var includeAllPrivateKeyProperties = new[]
        {
            nameof(TestCase.PrivateKey),
            nameof(TestCase.Seed)
        };

        var includeExpandedPrivateKeyProperties = new[]
        {
            nameof(TestCase.PrivateKey)
        };
        ...
        if (includeAllPrivateKeyProperties.Contains(jsonProperty.UnderlyingName, StringComparer.OrdinalIgnoreCase))
        {
            return jsonProperty.ShouldSerialize = instance =>
            {
                GetTestCaseFromTestCaseObject(instance, out var group, out var testCase);
                if (includeExpandedPrivateKeyProperties.Contains(jsonProperty.UnderlyingName,
