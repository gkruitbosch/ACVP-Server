...
namespace NIST.CVP.ACVTS.Libraries.Generation.Tests.DRBG.v1_0
{
    ...
    public class TestGroupGeneratorTests
    {
        ...
        [Test]
        [TestCase("ctrDRBG", "AES-128", 128)]
        [TestCase("ctrDRBG", "TDES", 64)] 
        [TestCase("hashDRBG", "SHA2-224", 0)]
        public async Task ShouldSetCounterFieldLengthCorrectly(string algorithm, string mode, int expectedCounterFieldLength)
        {
            var p = new Parameters()
            {
                Algorithm = algorithm,
            ...
            var results = await _subject.BuildTestGroupsAsync(p);
            Assert.That(results[0].DrbgParameters.CounterFieldLength, Is.EqualTo(expectedCounterFieldLength));
        }
